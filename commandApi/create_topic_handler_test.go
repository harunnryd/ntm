...
var _ = Describe("CreateTopicHandler", func() {
	var (
		h error
		e *echo.Echo
		c echo.Context
	...
	Context("when i call CreateTopicHandler", func() {
		BeforeEach(func() {
			e = echo.New()
			req = httptest.NewRequest(echo.POST, "/topics", strings.NewReader(`
				{
					"name": "unyil"
				}`))
